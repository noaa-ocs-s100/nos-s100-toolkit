...
RUN set -ex; \
    ...
    echo "Installing required system packages" \
      && apk --no-cache add gcc g++ make m4 musl-dev zlib zlib-dev gdal gdal-dev; \
    \
    echo "Downloading HDF5 source code" \
      ...
      && ./configure --prefix=/usr/local --with-zlib=/usr/include,/lib --enable-threadsafe --enable-unsupported --with-pthread=/usr/include,/usr/lib \
      && make \
      && make install \
      && cd .. \
      && rm ${hdf5_archive} \
...
RUN set -ex; \
    ...
    echo "Installing python packages" \
      && pip install -U pip \
      && pip install -e $PATH_S100_THYME \
      && pip install -e $PATH_S100_S100PY; \
    ...
    echo "Removing build packages" \
      && apk --no-cache del gcc g++ musl-dev make m4 zlib-dev;
